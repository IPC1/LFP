			...
			if ((etiqueta==true)&&(bandera==false)){
			  if(banderadiag==false){
				...
				if (banderadiag==true){//la etiqueta es de cierre.
					...
					}else{//album esta cerrado. el problema es con etiqueta final xml.
						xmlb=false;
						bandera=true;
					}
					}else{//xml esta cerrado. Las lineas no pertenecen a la lectura. O el problema es con etiqueta inicial xml.
						error+="Error sintactico3 en la linea: "+fila+". No se ha abierto la etiqueta XML. Caracteres fuera de lugar: "+caracter+"\n";
						bandera=true;
						palabra=null;
						etiqueta=false;
